...
void Renderer::init() {
    ...
    rigged_shader =
      RiggedShader("../src/shaders/rigged.vert", "../src/shaders/rigged.frag");
...
glm::vec2 Renderer::window_size() const noexcept {
    return window_size_;
}
...
glm::vec2 Renderer::camera_center() const noexcept {
    return camera_center_;
}

float Renderer::zoom_factor() const noexcept {
    return zoom_factor_;
}
