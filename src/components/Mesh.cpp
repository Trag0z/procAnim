...
RiggedMesh::RiggedMesh(const char* file) {
    ...
    for (size_t i = 0; i < meshData.mNumVertices; ++i) {
        vertices.push_back(
            {{vertData[i].x, vertData[i].y, 0.0f},
             {0.0f, 0.0f},
             {10.0f, 10.0f}, // So they reference the unity matrix by default
    ...
    for (uint i = 0; i < availableBones; ++i) {
        ...
        // memcpy(&boneOffsets[i], &b.mOffsetMatrix, sizeof(float) * 4 * 4);
        // boneOffsets[i] = glm::transpose(boneOffsets[i]);
    ...
    for (auto w : weightData) {
        ...
        if (boneCount == 2)
            continue;
        vertices[w.vertIndex].boneIndex[boneCount] =
            static_cast<float>(w.boneIndex);
